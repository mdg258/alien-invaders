...
class Bolt(GRectangle):
    ...
    def __init__(self, x, y, player):
        ...
        self.x = x
        self.y = y
        self.linecolor = WHITE_COLOR
        self.fillcolor = WHITE_COLOR
        self.width = BOLT_WIDTH
        self.height = BOLT_HEIGHT
        self.player = player
        self._velocity = self.isPlayerBolt()

    # GETTER TO SET THE BOLT VELOCITY
    def isPlayerBolt(self):
        """
        Detects if a bolt is a player bolt or an alien bolt
        and sets the bolt velocity accordingly.

        If the bolt is fired from an alien, it moves down: -BOLT_SPEED
        If the bolt is fired from the player, it moves up: BOLT_SPEED
        """
        if self.player:
            self._velocity = 14
        else:
...
class Heart(GImage):
    """
    A class to represent the game hearts.

    Hearts are used to represent player lives.
    """
    # INITIALIZER TO CREATE A SHIP
    def __init__(self, x, y, source):
        """
